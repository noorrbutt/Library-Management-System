...
class LibraryMembershipModelTest(TestCase):
    ...
    def test_str(self):
        membership = LibraryMembership.objects.create(
            library=self.library, user=self.member, role="member"
        )
        self.assertEqual(
            str(membership), f"{self.member.email} - Test Library (member)"
        )
    ...
    def test_unique_together(self):
        LibraryMembership.objects.create(library=self.library, user=self.member)
